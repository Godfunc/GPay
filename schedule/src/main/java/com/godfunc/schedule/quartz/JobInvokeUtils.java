...
import com.godfunc.schedule.utils.SpringUtils;
    ...
    public static Object invoke(JobEntity job) throws InvocationTargetException, IllegalAccessException, NoSuchMethodException, InstantiationException, ClassNotFoundException {
        ...
        if (invoke.contains("#")) {
            ...
            if (StringUtils.isNotBlank(paramsStr)) {
                ...
                Method invokeMethod = bean.getClass().getDeclaredMethod(method, params.keySet().toArray(new Class[0]));
                return invokeMethod.invoke(bean, params.values().toArray());
            }

        } else {
            int beanIndex = invoke.indexOf(".");
            int methodIndex = invoke.indexOf("(");
            int paramIndex = invoke.lastIndexOf(")");

            String beanName = invoke.substring(0, beanIndex);
            String method = invoke.substring(beanIndex + 1, methodIndex);
            String paramStr = invoke.substring(methodIndex + 1, paramIndex);
            Map<Class, Object> params = getParams(paramStr);
